using System;
using Microsoft.Xna.Framework.Input;

namespace GameMain.GameEngine.Input;

public class InputManager
{
    private readonly BaseInputMapper _inputMapper;

    public InputManager(BaseInputMapper inputMapper)
    {
        _inputMapper = inputMapper;
    }

    public void GetCommands(Action<BaseInputCommand> actOnState)
    {
        KeyboardState keyboardState = Keyboard.GetState();
        foreach (BaseInputCommand state in _inputMapper.GetKeyboardState(keyboardState))
        {
            actOnState(state);
        }

        MouseState mouseState = Mouse.GetState();
        foreach (BaseInputCommand state in _inputMapper.GetMouseState(mouseState))
        {
            actOnState(state);
        }

        // we're going to assume only 1 gamepad is being used
        GamePadState gamePadState = GamePad.GetState(0);
        foreach (BaseInputCommand state in _inputMapper.GetGamePadState(gamePadState))
        {
            actOnState(state);
        }
    }
}